...
class RationalsAddOrSubtractType2(Question):
    """
    """
    def __init__(self, **kwargs):
        ...
        if operator_symb == '-':
            ...
            op_fact = 1
        prob = '{t}'.format(t=sy.latex(sy.expand(term1)))
        prob += operator_symb
        prob += '\\frac{{{n}}}{{{d}}}'.format(n=sy.latex(sy.expand(numerator2)), d=sy.latex(sy.expand(denominator2)))

        self.answer = sy.simplify(term1 + op_fact*numerator2/denominator2)
    ...
    name = 'Rationals: Add or Subtract (Type 2)'
    module_name = 'rationals_add_or_subtract_type2'
...
Question_Class = RationalsAddOrSubtractType2
