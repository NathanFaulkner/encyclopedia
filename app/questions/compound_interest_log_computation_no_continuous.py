...
class CompoundInterestLogFormula(Question):
    def __init__(self, **kwargs):
        ...
        self.prompt_single = '''Say that you invest \\${P:,} at {r_pct}% annual interest, compounded
                {scheme}.  How many years will it take to earn \\${how_much:,}, assuming
    ...
    further_instruction = """Just give the numerical value.  You may round to 2 decimal places.
        ...
        "A = "; just enter the numerical part of your answer followed by a space and then the units (years).
    ...
    def checkanswer(self, user_answer):
        ...
        if self.scheme == 'daily':
            out = abs(user_answer - self.answer) < 0.005  or abs(user_answer - self.alt_answer) < 0.005 #and self.checkunits(user_units)
            return bool(out)
        else:
            out = abs(user_answer - self.answer) < 0.005 #and self.checkunits(user_units)
            return bool(out)
    ...
    def validator(user_answer):
        try:
            test_q = CompoundInterestLogFormula(seed=0)
            test_q.checkanswer(user_answer)
            test_q.format_useranswer(user_answer)
        except:
            raise SyntaxError
    # def validator(user_answer):
    #     try:
