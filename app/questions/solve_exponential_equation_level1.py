...
class SolveExponentialLevel1(Question):
    """
    Format is
    \[
    ...
    def __init__(self, **kwargs):
        ...
        if random.choice([True, False]):
            LHS, RHS = RHS, LHS
        self.answer = sy.simplify(sy.Rational(1, A)*(sy.log(d)/sy.log(b) + B))
        if (sy.log(d)/sy.log(b)).is_rational:
            self.format_answer = sy.latex(self.answer)
        else:
            if A == 1:
                if B == 0:
                    self.format_answer = f"\\log_{b}\\left({d}\\right)"
                elif B > 0:
                    self.format_answer = f"\\log_{b}\\left({d}\\right)+{B}"
                else:
                    self.format_answer = f"\\log_{b}\\left({d}\\right)-{abs(B)}"
            else:
                if B == 0:
                    self.format_answer = f"\\frac{{\\log_{b}\\left({d}\\right)}}{{ {A} }}"
                elif B > 0:
                    self.format_answer = f"\\frac{{\\log_{b}\\left({d}\\right)+{B}}}{{{A}}}"
                else:
                    self.format_answer = f"\\frac{{\\log_{b}\\left({d}\\right)-{abs(B)}}}{{{A}}}"
    ...
    module_name = 'solve_exponential_equation_level1'
...
Question_Class = SolveExponentialLevel1
