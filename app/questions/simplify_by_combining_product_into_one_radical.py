...
class SimplifyByCombiningProductIntoOneRadical(Question):
    """
    """
    def __init__(self, **kwargs):
        ...
        raised_b = b**q
        divisors = get_integer_divisors(b**q)
        divisors.remove(1)
        divisors.remove(b**q)
        r = random.choice(divisors)
        expr = f'{sy.latex(sy.Pow(int(raised_b/r),sy.Rational(1,q), evaluate=False))}\\cdot{sy.latex(sy.Pow(r,sy.Rational(1,q), evaluate=False))}'
        self.answer = b
        # print('self.answer:', self.answer)
        self.format_answer = f'\( {b} \)'
        self.format_given = f"""
        \\[
            {expr}
        \\]"""
    ...
    name = 'Simplify by Combining Into One Radical'
    module_name = 'simplify_by_combining_product_into_one_radical'
...
Question_Class = SimplifyByCombiningProductIntoOneRadical
