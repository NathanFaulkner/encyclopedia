...
class QuadraticOnlyOneX(Question):
    ...
    def __init__(self, **kwargs):
        ...
        if 'sqr' in kwargs:
            ...
            if self.has_solutions:
                self.sqr = random.randint(0,9)
            else:
                self.sqr = random.randint(-9,-1)
        if 'x' in kwargs:
            self.x = kwargs['x']
        else:
            self.x = sy.Symbol('x')
    ...
    \\[
        x = 5 - \\sqrt{3} \\quad \\textrm{or} \\quad x = 5 + \\sqrt{3}
    \\]
    You would enter, "5 - sqrt(3), 5 + sqrt(3)".
    """
