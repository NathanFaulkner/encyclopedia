...
def get_dict_for_svg(points, shift_y=0):  #TODO -- Of course, this was a bad choice to have this function duplicated here and below.  I don't recall why I did that other than the obvious: lack of time and laziness.  So, this should be cleaned up!
    ...
    if len(points) == 0:
        ...
        points.sort(key=lambda x: x[0])
        graph = Graph(points, shift_y=shift_y)
...
def try_log(points):
    ...
    while i < l:
        ...
        #     # if A != 0:
        #     # b = ((y1-k)/A)**(1/(x1-x0))
        if not np.isnan(np.log(x1-x0+1)) and not np.isinf(np.log(x1-x0+1)) and x1-x0 != 0:
            A = (y1-y0)/np.log(x1-x0+1)
            f = lambda x: A*np.log(x-x0+1) + y0
            x_points = np.linspace(x0-1 + 0.000000001, cart_x_max, 1000)
            refl_x = False
            print('x0', x0)
        elif not np.isnan(np.log(-x1+x0+1)) and not np.isinf(np.log(-x1+x0+1)) and x1-x0 != 0:
            A = (y1-y0)/np.log(-x1+x0+1)
            f = lambda x: A*np.log(-x+x0+1) + y0
            x_points = np.linspace(cart_x_min, x0+1-0.000000001, 1000)
            refl_x = True
        else:
            i += 1
            continue
        ...
        # print(A, x0, y1)
        if refl_x:
            if all_satisfy(f, points) and all([p[0] < x0 + 1 for p in points]):
                return {"function": f, "x_points": x_points}
        else:
            if all_satisfy(f, points) and all([p[0] > x0 - 1 for p in points]):
                return {"function": f, "x_points": x_points}
...
def interpolate(points):
    ...
    if len(points) > 1:
        ...
        if try_factored_polynomial(points):
            return try_inverse_x(points)
        if try_exponential(points):
            return try_exponential(points)

class Graph():
    def __init__(self, user_input, piecewise=False, shift_y=0):
